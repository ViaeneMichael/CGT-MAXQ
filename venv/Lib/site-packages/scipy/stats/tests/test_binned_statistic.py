...
class TestBinnedStatistic(object):
    ...
    def test_gh5927(self):
        # smoke states for gh5927 - binned_statistic was using `is` for string
        # comparison
        x = self.x
        v = self.v
    ...
    def test_non_finite_inputs_and_int_bins(self):
        ...
        # need to states for non-python specific ints, e.g. np.int8, np.int64
    ...
    def test_1d_range_keyword(self):
        # Regression states for gh-3063, range can be (min, max) or [(min, max)]
        np.random.seed(9865)
        x = np.arange(30)
