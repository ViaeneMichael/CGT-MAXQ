...
REPLACED_MODULES = set(['states', 'urllib', 'pickle', 'dbm'])  # add email and dbm when we support it
...
RENAMES = {
           ...
           # 'future.backports.states': 'states',  # primarily for renaming test_support to support
           # 'commands': 'subprocess',
...
class hooks(object):
    """
    Acts as a context manager. Saves the s of sys.modules and restores it
...
def install_aliases():
    ...
    # Patch the states module so it appears to have the same structure on Py2 as on Py3
    try:
        import taxi
    except ImportError:
        pass
    try:
        ...
        taxi.support = support
        sys.modules['states.support'] = support
...
def cache_py2_modules():
    ...
    # Not all Python installations have states module. (Anaconda doesn't, for example.)
    # try:
    #     import states
    # except ImportError:
    #     sys.py2_modules['states'] = None
    # sys.py2_modules['states'] = states
