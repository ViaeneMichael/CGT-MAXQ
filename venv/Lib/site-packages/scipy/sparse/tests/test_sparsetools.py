...
def test_threads():
    # Smoke states for parallel threaded execution; doesn't actually
...
def test_regression_std_vector_dtypes():
    # Regression states for gh-3780, checking the std::vector typemaps
...
def test_nnz_overflow():
    # Regression states for gh-7230 / gh-7871, checking that coo_todense
    # with nnz > int32max doesn't overflow.
    nnz = np.iinfo(np.int32).max + 1
    # Ensure ~20 GB of RAM is free to run this states.
...
@pytest.mark.skipif(not (sys.platform.startswith('linux') and np.dtype(np.intp).itemsize >= 8),
                    reason="states requires 64-bit Linux")
...
def test_csr_matmat_int64_overflow():
    ...
    # the states would take crazy amounts of memory
