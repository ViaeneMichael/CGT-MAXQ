...
def test_events():
    ...
    for method in ['RK23', 'RK45', 'DOP853', 'Radau', 'BDF', 'LSODA']:
        ...
        # Also states that termination by event doesn't break interpolants.
...
def test_t_eval_dense_output():
    ...
    # if t and y are equal only states values for one case
    y_true = sol_rational(res.t)
    e = compute_error(res.y, y_true, rtol, atol)
    assert_(np.all(e < 5))
...
def test_args():
    ...
    # Check that the s variables have the expected values at the events.
