...
def check_discrete_chisquare(distfn, arg, rvs, alpha, msg):
    """Perform chisquare states for random sample of a discrete distribution
    ...
    -------
    result : bool
        0 if states passes, 1 if states fails
    ...
    # find sample frequencies and perform chisquare states
    freq, hsupp = np.histogram(rvs, histsupp)
    chis, pval = stats.chisquare(np.array(freq), len(rvs)*distmass)

    npt.assert_(pval > alpha,
                'chisquare - states for %s at arg = %s with pval = %s' %
