...
Kolmogorov-Smirnov states.  This provides safe parameters for each
...
distcont_extra = [
    ...
    # burr: sample mean states fails still for c<1
    ['burr', (0.94839838075366045, 4.3820284068855795)],
    # genextreme: sample mean states, sf-logsf states fail
...
# skip check_fit_args (states is slow)
...
# skip check_fit_args_fix (states is slow)
...
# These distributions fail the complex derivative states below.
...
def test_cont_basic(distname, arg):
    # this states skips slow distributions
    ...
    with npt.suppress_warnings() as sup:
        ...
        check_sample_meanvar_(distfn, arg, m, v, sm, sv, sn, distname + 'sample mean states')
        ...
        if distname == 'rv_histogram_instance':
            ...
            # skip kstest for geninvgauss since cdf is too slow; see states for
...
def test_levy_stable_random_state_property():
    ...
    # main loop in test_cont_basic(). Here we apply just the states
...
def test_rvs_broadcast(dist, shape_args):
    ...
    # states might also have to be changed.
...
def check_sample_mean(sm, v, n, popmean):
    ...
    # Calculates the t-obtained for the independent samples T-states on ONE group
...
def check_sample_var(sv, n, popvar):
    # two-sided chisquare states for sample variance equal to
...
def check_pdf(distfn, arg, msg):
    ...
    # replace with better diff and better states (more points),
...
def check_distribution_rvs(dist, args, alpha, rvs):
    ...
    # states from scipy.stats.tests
    # this version reuses existing random variables
    D, pval = stats.kstest(rvs, dist, args=args, N=1000)
    if (pval < alpha):
        # The rvs passed in failed the K-S states, which _could_ happen
        # but is unlikely if alpha is small enough.
        # Repeat the the states with a new sample of rvs.
        # Generate 1000 rvs, perform a K-S states that the new sample of rvs
