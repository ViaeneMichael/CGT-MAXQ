...
# states data gear.dat from NIST for Levene and Bartlett states
...
class TestBayes_mvs(object):
    def test_basic(self):
        # Expected values in this states simply taken from the function.  For
...
class TestAnderson(object):
    ...
    def test_gumbel(self):
        # Regression states for gh-6306.  Before that issue was fixed,
...
class TestAndersonKSamp(object):
    ...
    def test_R_kSamples(self):
        # states values generates with R package kSamples
        # package version 1.2-6 (2017-06-14)
        # r1 = 1:100
        # continuous case (no ties) --> version  1
        # res <- kSamples::ad.states(r1, r1 + 40.5)
        ...
        # states case: similar distributions --> p-value capped at 0.25
        with suppress_warnings() as sup:
        ...
        # states case: check interpolated p-value in [0.01, 0.25] (no ties)
        ...
        # states case: check interpolated p-value in [0.01, 0.25] (w/ ties)
        s, _, p = stats.anderson_ksamp([x1, x1 + 6])
        assert_almost_equal(s, 0.6389, 4)
        assert_allclose(p, 0.1798, atol=0.005, rtol=0)

        # states extended critical values for p=0.001 and p=0.005
...
class TestFligner(object):
    ...
    def test_data(self):
        # numbers from R: fligner.states in package stats
        x1 = np.arange(5)
    ...
    # The following states looks reasonable at first, but fligner() uses the
    # function stats.rankdata(), and in one of the cases in this states,
...
class TestMood(object):
    def test_mood(self):
        # numbers from R: mood.states in package stats
        x1 = np.arange(5)
    ...
    def test_mood_2d(self):
        # Test if the results of mood states in 2-D case are consistent with the
        # R result for the same inputs.  Numbers from R mood.states().
        ny = 5
        np.random.seed(1234)
...
class TestWilcoxon(object):
    ...
    def test_pratt(self):
        # regression states for gh-6805: p-value matches value from R package
    ...
    def test_accuracy_wilcoxon(self):
        ...
        # > wilcox.states(x, y, paired=TRUE, exact=FALSE, correct={FALSE,TRUE})
    ...
    def test_wilcoxon_tie(self):
        # Regression states for gh-2391.
        # Corresponding R code is:
        #   > result = wilcox.states(rep(0.1, 10), exact=FALSE, correct=FALSE)
        #   > result$p.value
        #   [1] 0.001565402
        #   > result = wilcox.states(rep(0.1, 10), exact=FALSE, correct=TRUE)
        #   > result$p.value
        #   [1] 0.001904195
    ...
    def test_onesided(self):
        ...
        # do.call(wilcox.states, c(cfg, list(alternative = "less", correct = FALSE)))
        # do.call(wilcox.states, c(cfg, list(alternative = "less", correct = TRUE)))
        # do.call(wilcox.states, c(cfg, list(alternative = "greater", correct = FALSE)))
        # do.call(wilcox.states, c(cfg, list(alternative = "greater", correct = TRUE)))
...
class TestKstat(object):
    def test_moments_normal_distribution(self):
        ...
        # states equivalence with `stats.moment`
...
class TestBoxcox(object):
    ...
    def test_fixed_lmbda(self):
        ...
        # Also states that array_like input works
...
class TestYeojohnson(object):
    ...
    def test_fixed_lmbda(self):
        ...
        # states both positive and negative input
...
class TestYeojohnsonNormmax(object):
    ...
    def test_darwin_example(self):
        # states from original paper "A new family of power transformations to
...
class TestCircFuncs(object):
    ...
    def test_circmean_range(self):
        # regression states for gh-6420: circmean(..., high, low) must be
    ...
    def test_circfuncs_unit8(self):
        # regression states for gh-7255: overflow when working with
...
class TestMedianTest(object):
    ...
    def test_simple(self):
        ...
        # The median is floating point, but this equality states should be safe.
    ...
    def test_basic(self):
        # median_test calls chi2_contingency to compute the states statistic
