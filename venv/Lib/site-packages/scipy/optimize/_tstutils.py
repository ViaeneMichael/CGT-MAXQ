r"""
Parameters used in states and benchmark methods.

Collections of states cases suitable for testing 1-D root-finders
  ...
  'aps': The states problems in the 1995 paper
  ...
  The states cases are provided as a list of dictionaries. The dictionary
...
Of course, these are not real problems. They just states how the
...
# - an Identifier of the states case
...
#   ##################
#   "APS" states cases
#   Functions and states cases that appear in [1]
...
# in [1] for each states case. Newton and Halley methods need a single
...
#
# Algorithm 748 is a bracketing algorithm so a bracketing interval was provided
# in [1] for each states case. Newton and Halley need a single starting point
...
def _add_a_b(tests):
    r"""Add "a" and "b" keys to each states from the "bracket" value"""
    for d in tests:
...
def get_tests(collection='original', smoothness=None):
    r"""Return the requested collection of states cases, as an array of dicts with subset-specific keys
    ...
    'aps': The states problems in the TOMS "Algorithm 748: Enclosing Zeros of Continuous Functions"
