    ...
    try:
        for pre_state in (True, False):
            ...
            for with_state in (True, False):
                # Check the gc s is with_state in with block
                with gc_state(with_state):
                    assert_equal(gc.isenabled(), with_state)
                # And returns to previous s outside block
                assert_equal(gc.isenabled(), pre_state)
                # Even if the gc s is set explicitly within the block
                with gc_state(with_state):
                    assert_equal(gc.isenabled(), with_state)
...
def test_assert_deallocated_nodel():
    ...
    with pytest.raises(ReferenceError):
        ...
        # Note: assert_deallocated(C) needs to be assigned for the states
