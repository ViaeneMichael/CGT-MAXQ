...
def ImmediateReward(task, state):
  ...
  return maxqGraph.getImmediateReward(task, bInvalid)
...
def IsTerminalState(i, state):
  ...
  if maxqGraph.isPrimitiveAction(task):
...
def EVALUATEMAXNODE(i, s):
  if maxqGraph.isPrimitiveAction(i):
    ...
    v_opt = - sys.maxint
    a_opt = None
    for j in maxqGraph.getAvailableActions(i, task_bound):
...
def EXECUTEHGPOLICY(s):
  while True:
    v, a = EVALUATEMAXNODE((task2Action.Task_Root, None), s)
...
def MAXQ_Q(i, s):
  ...
  if maxqGraph.isPrimitiveAction(i):  # primitive MaxNode
    ...
    while not IsTerminalState(i, s):
      ...
      # here we need observe result state s' , saved in global variable s_prime
      ...
      for _s in childSeq:
        ...
        CTildevalues[(i, _s, a)] = (1 - alpha) * CTildevalues[(i, _s, a)] + alpha * d * (
                  R_tilde(i, s_prime) + Q_tilde(i, s_prime, a_opt))
...
class AbstractVvalues(defaultdict):
  def state_abstract(self, i, s):
    ...
    if task in (task2Action.Task_South, task2Action.Task_North, task2Action.Task_East, task2Action.Task_West):
      s = -1
    
    # abs2: Pickup and Putdown each require 2 values (legal and illegal states),
    elif task == task2Action.Task_Pickup:
      taxirow, taxicol, passidx, destidx = decode(s)
      s = -2 if passidx >= 4 or (taxirow, taxicol) != locs[passidx] else -1
    
    elif task == task2Action.Task_Putdown:
      taxirow, taxicol, passidx, destidx = decode(s)
...
class AbstractCvalues(defaultdict):
  def state_abstract(self, i, s, a):
    ...
    # p
    if task == task2Action.Task_Navigate:
      # t is bounded in i
    ...
    elif task == task2Action.Task_Get and subtask == task2Action.Task_Navigate:
    ...
    # p
    elif task == task2Action.Task_Get and subtask == task2Action.Task_Pickup:
    ...
    # p
    elif task == task2Action.Task_Put and subtask == task2Action.Task_Putdown:
      s = encode(taxirow, taxicol, 0, destidx)
    
    # QPut requires 0 values. (Termination and Shielding.)
    # p
    elif subtask == task2Action.Task_Put:  # C(Root, s, Put)
      s = -1
    return (i, s, a)
...
if __name__ == '__main__':
  ...
  for task in maxqGraph.getMaxNodes():
  ...
  action2Task = dict((v, k) for k, v in enumerate(task2Action))
