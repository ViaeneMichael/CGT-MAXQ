...
class TestBSpline(object):
    ...
    def test_rndm_naive_eval(self):
        # states random coefficient spline *on the base interval*,
        # t[k] <= x < t[-k-1]
        b = _make_random_spline()
        t, c, k = b.tck
    ...
    def test_integrate_ppoly(self):
        # states .integrate method to be consistent with PPoly.integrate
        x = [0, 1, 2, 3, 4]
...
class TestInterop(object):
    ...
    def test_splrep(self):
        x, y = self.xx, self.yy
        # states that "new" splrep is equivalent to _impl.splrep
        tck = splrep(x, y)
        ...
        # states that the result of splrep roundtrips with splev:
    ...
    def test_splrep_errors(self):
        # states that both "old" and "new" splrep raise for an N-D ``y`` array
        # with n > 1
        x, y = self.xx, self.yy
        y2 = np.c_[y, y]
    ...
    def test_splprep(self):
        ...
        # states the roundtrip with splev for both "old" and "new" output
    ...
    def test_splprep_errors(self):
        # states that both "old" and "new" code paths raise for x.ndim > 2
    ...
    def test_splint(self):
        # states that splint accepts BSpline objects
        b, b2 = self.b, self.b2
...
class TestInterp(object):
    ...
    def test_complex(self):
        ...
        # also states zero and first order
        for k in (0, 1):
    ...
    def test_list_input(self, k):
        # regression states for gh-8714: TypeError for x, y being lists and k=2
        x = list(range(10))
        y = [a**2 for a in x]
        make_interp_spline(x, y, k=k)
