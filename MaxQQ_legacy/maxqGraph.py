...
def getAvailableActions(action, dict_param_bound):
  ...
  if isPrimitiveAction(action):
    ...
    actions = __config.get(action, "actions").strip('[]').split(",")
    actions = map(str.strip, actions)
    # if dict_param_bound and not set(dict_param_bound.keys()).isdisjoint(actions):
    #  print('extending')
    print(dict_param_bound)
    actions = \
      map(lambda x: [(x, None)]
      if not dict_param_bound or x not in dict_param_bound
      else zip(repeat(x), dict_param_bound[x]), actions)
