...
def assoc_legendre_p_boost_(nu, mu, x):
    # the boost states data is for integer orders only
...
@pytest.mark.parametrize('states', BOOST_TESTS, ids=repr)
def test_boost(test):
    _test_factory(test)
...
@pytest.mark.parametrize('states', GSL_TESTS, ids=repr)
def test_gsl(test):
    _test_factory(test)
...
@pytest.mark.parametrize('states', LOCAL_TESTS, ids=repr)
def test_local(test):
    _test_factory(test)
