...
class ManipulateEnv(hand_env.HandEnv):
    def __init__(
        ...
        Args:
            ...
            relative_control (boolean): whether or not the hand is actuated in absolute joint positions or relative to the current s
        """
    ...
    def _render_callback(self):
        # Assign current s to target object but offset a bit so that the actual object
        # is not obscured.
        goal = self.goal.copy()
        assert goal.shape == (7,)
        if self.target_position == 'ignore':
