...
def make_simple(*args, **kwargs):
    ...
    f.history = 'Created for a states'
...
def check_simple(ncfileobj):
    ...
    assert_equal(ncfileobj.history, b'Created for a states')
...
def test_read_write_files():
    # states round trip for example file
    cwd = os.getcwd()
    try:
...
def test_itemset_no_segfault_on_readonly():
    # Regression states for ticket #1202.
    # Open the states file in read-only mode.
...
def test_ticket_1720():
    ...
    with netcdf_file(io, 'r') as f:
        assert_equal(f.history, b'Created for a states')
...
def test_read_withValuesNearFillValue():
    # Regression states for ticket #5626
