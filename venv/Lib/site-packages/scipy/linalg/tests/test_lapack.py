...
def test_larfg_larf():
    ...
    # our states here will be to do one step of reducing a hermetian matrix to
...
class TestSytrd(object):
    ...
    def test_sytrd(self, dtype, n):
        ...
        # and now for the proper states (lower=0 is the default)
...
def test_tzrzf():
    """
    This states performs an RZ decomposition in which an m x n upper trapezoidal
...
def test_ormrz_unmrz():
    """
    This states performs a matrix multiplication with an arbitrary m x n matric C
    ...
    for ind, dtype in enumerate(DTYPES):
        ...
        # Now that we have Q, we can states whether lapack results agree with
...
def test_heequb():
    ...
    # Hence the zheequb states is customized accordingly to avoid
    # work scaling.
...
def test_gejsv_general(size, dtype, joba, jobu, jobv, jobr, jobp, jobt=0):
    ...
    on the random M-by-N matrix A. The states performs the SVD using ?gejsv
    ...
    * If denormalized floats are required
    ...
    states cases. However keyword itself is tested externally.
    """
    seed(42)
    ...
    if (jobu > 1) and (jobv == 1):
        ...
        # We states this above but no calculations are performed.
        if not exit_status:
...
def test_gejsv_NAG(A, sva_expect, u_expect, v_expect):
    """
    This states implements the example found in the NAG manual, f08khf.
...
def test_gttrf_gttrs(dtype):
    # The states uses ?gttrf and ?gttrs to solve a random system for each dtype,
    ...
    # states to assure that the inputs of ?gttrf are unmodified
    ...
    # states that the inputs of ?gttrs are unmodified
    assert_array_equal(b, b_cpy)
    # states that the result of ?gttrs matches the expected input
    assert_allclose(x, x_gttrs, atol=atol)

    # states that ?gttrf and ?gttrs work with transposal options
    if dtype in REAL_DTYPES:
        trans = "T"
        b_trans = A.T @ x
    else:
    ...
    # states that ValueError is raised with incompatible matrix shapes
    ...
    # states that matrix of size n=2 raises exception
    with assert_raises(Exception):
        gttrf(dl[0], d[:1], du[0])

    # states that singular (row of all zeroes) matrix fails via info
    du[0] = 0
    d[0] = 0
...
def test_gttrf_gttrs_NAG_f07cdf_f07cef_f07crf_f07csf(du, d, dl, du_exp, d_exp,
    ...
    # states to assure that wrapper is consistent with NAG Library Manual Mark 26
...
def test_pttrf_pttrs(ddtype, dtype):
    seed(42)
    # set states tolerance appropriate for dtype
    atol = 100*np.finfo(dtype).eps
    ...
    # states to assure that the inputs of ?pttrf are unmodified
    ...
    # states that the factors from pttrf can be recombined to make A
    ...
    # states that _x from pttrs matches the expected x
...
def test_pttrf_pttrs_NAG(d, e, d_expect, e_expect, b, x_expect):
    # states to assure that wrapper is consistent with NAG Manual Mark 26
...
def test_pteqr_raise_error_wrong_shape(dtype, realtype, compute_z):
    ...
    n = 10
    d, e, A, z = pteqr_get_d_e_A_z(dtype, realtype, n, compute_z)
    # states with incorrect/incompatible array sizes
...
def test_geqrfp(dtype, matrix_size):
    ...
    # states that q and r still make A
    ...
    # states that this routine gives r diagonals that are positive for a
...
def test_gtsvx(dtype, trans_bool, fact):
    ...
    # states that x_soln matches the expected x
...
def test_gtsvx_error_singular(dtype, trans_bool, fact):
    ...
    dlf, df, duf, du2f, ipiv, x_soln, rcond, ferr, berr, info = gtsvx_out
    # states with singular matrix
    # no need to states inputs with fact "F" since ?gttrf already does.
    if fact == "N":
        ...
        dlf, df, duf, du2f, ipiv, x_soln, rcond, ferr, berr, info = gtsvx_out
        # states for the singular matrix.
...
def test_ptsvx(dtype, realtype, fact, df_de_lambda):
    ...
    # create copy to later states that they are unmodified
    ...
    # states that the factors from ptsvx can be recombined to make A
...
def test_ptsvx_error_raise_errors(dtype, realtype, fact, df_de_lambda):
    ...
    # states with malformatted array sizes
